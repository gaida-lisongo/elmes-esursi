import { Quota } from "@/types/annee";

type FaqData = {
    activeFaq: string;
    id: string;
    handleFaqToggle: (id: string) => void;
    quota: Quota;
};

const FAQItem = ({ faqData }: { faqData: FaqData }) => {
    const { activeFaq, id, handleFaqToggle, quota } = faqData;

    return (
        <>
            <div className="flex flex-col border-b border-stroke last-of-type:border-none dark:border-strokedark">
                <button
                    onClick={() => {
                        handleFaqToggle(id);
                    }}
                    className="flex cursor-pointer items-center justify-between px-6 py-5 text-metatitle3 font-medium text-black dark:text-white lg:px-9 lg:py-7.5"
                >
                    <div className="flex justify-between w-full pr-4">
                        <span>{quota.designation}</span>
                        <span>{quota.montant} $</span>
                    </div>

                    {activeFaq === id ? (
                        <svg
                            width="18"
                            height="4"
                            viewBox="0 0 18 4"
                            fill="none"
                            xmlns="http://www.w3.org/2000/svg"
                        >
                            <path
                                d="M17.1666 0.833374H10.1666H7.83331H0.833313V3.16671H7.83331H10.1666H17.1666V0.833374Z"
                                fill="currentColor"
                            />
                        </svg>
                    ) : (
                        <svg
                            width="18"
                            height="18"
                            viewBox="0 0 18 18"
                            fill="none"
                            xmlns="http://www.w3.org/2000/svg"
                        >
                            <path
                                d="M7.83331 7.83337V0.833374H10.1666V7.83337H17.1666V10.1667H10.1666V17.1667H7.83331V10.1667H0.833313V7.83337H7.83331Z"
                                fill="currentColor"
                            />
                        </svg>
                    )}
                </button>
                <div
                    className={`border-t border-stroke px-6 py-5 dark:border-strokedark lg:px-9 lg:py-7.5 ${activeFaq === id ? "block" : "hidden"
                        }`}
                >
                    {/* Check if description exists and has items */}
                    {quota.description && quota.description.length > 0 ? (
                        <ul className="list-disc pl-5 space-y-1">
                            {quota.description.map((desc, idx) => (
                                <li key={idx} className="text-sm text-gray-600 dark:text-gray-300">
                                    {desc}
                                </li>
                            ))}
                        </ul>
                    ) : (
                        <p className="text-sm text-gray-500">Pas de détails supplémentaires.</p>
                    )}
                </div>
            </div>
        </>
    );
};

export default FAQItem;
