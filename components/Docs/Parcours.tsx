...
const ParcoursSubscriber = ({ annee, etudiant, programme, etablissement }: { annee: any, etudiant: any, programme: any, etablissement: any }) => {
    ...
    const [formParcour, setFormParcours] = useState({
    ...
    const steps = [
        ...
        },
        {
            id: 2, title: "Paiement", icon: (
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="SearchPath-5 8h10M5 8a2 2 0 110-4h14a2 2 0 110 4M5 8v10a2 2 0 002 2h10a2 2 0 002-2V8m-9 4h4" />
                </svg>
            )
        },
        {
            id: 3, title: "Vérification", icon: (
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {
            id: 4, title: "Félicitations", icon: (
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="Children 22 2c0 2.761-2.239 5-5 5S12 4.761 12 2" />
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M14.5 17.5L12 15l-2.5 2.5" />
                </svg>
            )
        }
    ];
    ...
    const handleCreateParcoursAction = async () => {
        ...
        try {
            ...
            if (req.success) {
                ...
                toast.error(req.message || "Erreur lors de la création du parcours");
            }
        } catch (error) {
    ...
    const handlePaiementTrigger = async () => {
        ...
        try {
            ...
            if (req.success) {
                ...
                toast.error(req.message || "Erreur lors de l'initiation du paiement");
            }
        } catch (error) {
    ...
    const handleCheckPaiement = async () => {
        ...
        try {
            ...
            if (req.success && req.check) {
                setCheck(true);
                // Si paiement réussi, on met à jour le parcours
                await handleUpdateParcoursAction();
            } else {
                toast.error("Le paiement n'est pas encore confirmé. Veuillez patienter.");
            }
        } catch (error) {
                        ...
                        <div className="space-y-8">
                            ...
                            <div className="grid md:grid-cols-2 gap-8">
                                <div className="space-y-6">
                                    ...
                                    <SectionInfo
                                        ...
                                        subValue={etablissement?.province}
                                        icon={<SchoolIcon />}
                                    />
                                </div>
                                <div className="space-y-6">
                            ...
                            <div className="mt-8 p-6 bg-blue-50 dark:bg-blue-900/20 rounded-2xl border border-blue-100 dark:border-blue-800">
                                <div className="flex justify-between items-center">
                                    <span className="text-gray-700 dark:text-gray-300 font-medium text-lg">Frais de traitement du dossier</span>
                            ...
                            <div className="max-w-md mx-auto space-y-4">
                                ...
                                <div className="p-4 bg-amber-50 dark:bg-amber-900/20 rounded-xl border border-amber-100 dark:border-amber-800 flex gap-3">
                                    ...
                                    <p className="text-sm text-amber-800 dark:text-amber-300">
                                        Assurez-vous que le téléphone est à proximité pour valider le retrait une fois la demande envoyée.
                                    </p>
                                </div>
                                ...
                                <button
                                    ...
                                    className="w-full py-2 text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 font-medium transition-colors"
                                >
                                    Retour au résumé
                                </button>
                            </div>
                        ...
                        <div className="space-y-8 text-center py-4">
                            ...
                            <div className="max-w-md mx-auto space-y-4">
                                <button
                                    ...
                                    className="w-full py-4 px-6 bg-primary hover:bg-primary/90 text-white rounded-2xl font-bold text-lg transition-all transform hover:scale-[1.02] active:scale-[0.98] disabled:opacity-70 disabled:cursor-not-allowed shadow-lg shadow-primary/25 flex items-center justify-center gap-3"
                                >
                                    {loading ? <Spinner /> : "Vérifier le statut"}
                                </button>
                    ...
                    {currentStep === 4 && (
                        <div className="space-y-8 text-center pt-4">
                            <motion.div
                                ...
                                className="w-24 h-24 bg-green-500 rounded-full mx-auto flex items-center justify-center mb-6 shadow-xl shadow-green-500/20"
                            >
                            ...
                            <div className="bg-gray-50 dark:bg-gray-900 rounded-2xl p-6 border border-gray-100 dark:border-gray-800 max-w-sm mx-auto text-left">
                                <div className="flex justify-between items-center mb-4 pb-4 border-b border-gray-200 dark:border-gray-800">
                                    <span className="text-gray-400 text-sm">Status</span>
                                    <span className="px-3 py-1 bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400 text-xs font-bold rounded-full uppercase">Confirmé</span>
                                </div>
                                <div className="space-y-3">
                                    <div className="flex justify-between">
                                        <span className="text-gray-400 text-sm">Référence</span>
                                        <span className="text-gray-900 dark:text-white font-mono text-sm">{orderNumber}</span>
                                    </div>
                                    <div className="flex justify-between">
                                        <span className="text-gray-400 text-sm">Montant</span>
                                        <span className="text-gray-900 dark:text-white font-bold">{formPaiement.amount.toLocaleString()} {formPaiement.currency}</span>
                                    </div>
                                    <div className="flex justify-between">
                            ...
                            <div className="pt-4 flex flex-col md:flex-row gap-4 justify-center">
                                ...
                                <button className="px-8 py-3 bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-white rounded-xl font-bold hover:bg-gray-300 dark:hover:bg-gray-600 transition-all text-sm">
                                    Télécharger le reçu
                                </button>
                            </div>
                        </div>
